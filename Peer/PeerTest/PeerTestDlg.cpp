...
//		peerCreateStagingRoom(m_peer, m_name, 6, m_password, JoinRoomCallback, NULL, (PEERBool)m_blocking);
		if(m_peer)
		{
			SOCKADDR_IN localAddr; // used if we are calling with a socket
			...
			// setup socket for calling peerCreateStagingRoomWithSocket
			...
			peerCreateStagingRoomWithSocket(m_peer, m_name, 6, m_password, m_Socket, NET_PORT, JoinRoomCallback, NULL, (PEERBool)m_blocking);
		}
	}
}
