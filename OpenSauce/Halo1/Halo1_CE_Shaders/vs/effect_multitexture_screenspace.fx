...
VS_OUTPUT main(float4 v9 : COLOR0, float4 v0 : POSITION0, float4 v4 : TEXCOORD0)
{
	...
	// (7) output texcoords ------------------------------------------------------------------
	o.T0.xy = V_TEXCOORD;	
	R_TEMP0.x = dot(R_POSITION, c_world_view_projection[0]);	
	R_TEMP0.y = dot(R_POSITION, c_world_view_projection[1]);	
	R_TEMP0.zw = dot(R_POSITION, c_world_view_projection[3]);	
	o.T0.z = dot(R_TEMP0, c_texture1_xform_x);	
	o.T0.w = dot(R_TEMP0, c_texture1_xform_y);
...
Technique effect_multitexture_screenspace
{
	Pass P0
	{
#ifndef USE_ASM
