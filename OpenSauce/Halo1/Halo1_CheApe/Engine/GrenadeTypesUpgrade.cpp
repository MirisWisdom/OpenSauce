...
namespace Yelo
{
	...
	namespace Objects { namespace Items {
		...
		static tag_field* scenario_profiles_block_find_unused_grenade_count_fields()
		{
			...
			field_index = scnr->header_block_definition->find_field_index(e_field_type::block, "player starting profile");
			if(field_index == NONE)
			{
			...
			auto* scenario_profiles_block_def = scnr->header_block_definition->fields[field_index].get_definition<tag_block_definition>();
			field_index = scenario_profiles_block_def->find_field_index(e_field_type::char_integer, k_scenario_profiles_block_unused_grenade_count_field_name);
			if(field_index == NONE)
			{
