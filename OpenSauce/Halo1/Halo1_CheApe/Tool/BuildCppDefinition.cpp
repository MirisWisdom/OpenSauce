...
namespace Yelo
{
	namespace Tool
	{
			...
			cstring name)
		{
			...
			TagGroups::group_tag_to_string gt;
			gt.Terminate();
			...
			cstring description)
		{
			...
			if (enum_instance)
				fprintf_s(file, ", %s::type_t", enum_instance->GetName().c_str());
			if (description)
				fprintf_s(file, ", \"%s\"", description);
			...
			c_string_list_instance& instance)
		{
			...
			fprintf(file, "\t\tnamespace e_%s\n", field_name.c_str());
			fputs("\t\t{\n", file);
			fputs("\t\t\ttypedef enum : _enum\n", file);
			fputs("\t\t\t{\n", file);
			...
			const bool add_boost_asserts)
		{
			...
			{
				fprintf(file,
					"\n\n\t\t\tstatic_assert( sizeof(%s) == sizeof(%s), \"%s should be the same size as %s\" );",
					field_name.c_str(),
					type,
					field_name.c_str(),
					type);
			...
			const bool add_boost_asserts)
		{
			...
			const tag_block_definition* block_definition = instance.GetDefinition();
			...
			if (add_boost_asserts)
				fprintf_s(file,
					"\t\t};\n\n\t\t// max count: %i\n\t\tstatic_assert( sizeof(%s) == 0x%X, \"%s should be %d bytes in length\" );\n",
					block_definition->maximum_element_count,
					block_name.c_str(), block_definition->element_size, block_name.c_str(), block_definition->element_size);
			else
