...
// GameUI.cpp
	...
	namespace Hud
	{
		...
		ENGINE_PTR(uint32, HUD_RENDER_NAV_POINT_ANCHOR_HALF_X,		0x4B2D11, PTR_NULL);
		ENGINE_PTR(uint32, HUD_RENDER_NAV_POINT_ANCHOR_HALF_Y,		0x4B2D1B, PTR_NULL);
		...
		#pragma region hud
		ENGINE_PTR(uint32, HUD_POINT_ANCHOR_WIDTH,					0x4AED20, PTR_NULL);
		ENGINE_PTR(uint32, HUD_POINT_ANCHOR_HEIGHT,					0x4AED52, PTR_NULL);
		ENGINE_PTR(uint32, HUD_POINT_ANCHOR_HALF_WIDTH,				0x4AED72, PTR_NULL);
		ENGINE_PTR(uint32, HUD_POINT_ANCHOR_HALF_HEIGHT,			0x4AED96, PTR_NULL);

		ENGINE_PTR(uint32, HUD_POINT_DAMAGE_ANCHOR_WIDTH,			0x4B4B95, PTR_NULL);
		ENGINE_PTR(uint32, HUD_POINT_DAMAGE_ANCHOR_HEIGHT,			0x4B4B6E, PTR_NULL);
		ENGINE_PTR(real, HUD_POINT_DAMAGE_ANCHOR_HALF_HEIGHT,		0x4B4BAF, PTR_NULL);

		static void** K_HUD_POINT_DAMAGE_ANCHOR_HALF_WIDTH_PTRS[] =
		{
			CAST_PTR(void**, 0x4B4B8A), CAST_PTR(void**, 0x4B4B4A)
		};
		#pragma endregion
