            ...
            namespace Generic
            {
                namespace External
                {
                    void c_shader_external::SetShaderPath(const char* path)
                    {
                        ...
                        auto filename_offset = strrchr(path, '\\');
                        if (!filename_offset)
                        {
                            return;
                        }

                        auto index = CAST(uint32, filename_offset - path);
                    ...
                    void c_shader_external::Ctor()
                    {
                        c_shader_generic::Ctor();

                        m_members_external.shader_path[0] = 0;
                        m_members_external.shader_id[0] = '\0';
                        m_members_external.shader_source_data = nullptr;
                    }

                    void c_shader_external::Dtor()
                    {
                        m_members_external.shader_path[0] = 0;
                        m_members_external.shader_id[0] = '\0';
