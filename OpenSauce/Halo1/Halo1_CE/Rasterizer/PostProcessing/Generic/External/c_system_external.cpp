...
#pragma region External PP Settings
                    class c_shader_container : public Configuration::c_configuration_container
                    {
                    public:
                        ...
                        c_shader_container()
                            : c_configuration_container("Shader"),
                              m_id("ID", ""),
                              m_file("File", "") { }
                    ...
                    protected:
                        ...
                        {
                            return std::vector<i_configuration_value* const> { &m_id, &m_file };
                        }
                    };

                    class c_effect_container : public Configuration::c_configuration_container
                    {
                    public:
                        Configuration::c_configuration_value<std::string> m_id;
                    ...
                    protected:
                        const std::vector<i_configuration_value* const> GetMembers() final override
                        {
                            return std::vector<i_configuration_value* const> { &m_id, &m_shader_ids };
                        }
                    };

                    class c_quad_container : public Configuration::c_configuration_container
                    {
                    public:
                        ...
                        c_quad_container()
                            : c_configuration_container("Quad"),
                    ...
                    protected:
                        const std::vector<i_configuration_value* const> GetMembers() final override
                        {
                            return std::vector<i_configuration_value* const> { &m_tessellation, &m_bounds_x, &m_bounds_y };
                        }
                    };

                    class c_activation_control_container : public Configuration::c_configuration_container
                    {
                    public:
                        ...
                        c_activation_control_container()
                            : c_configuration_container("ActivationControl"),
                    ...
                    protected:
                        const std::vector<i_configuration_value* const> GetMembers() final override
                        {
                            return std::vector<i_configuration_value* const> { &m_activation_state, &m_invert };
                        }
                    };

                    class c_effect_instance_container : public Configuration::c_configuration_container
                    {
                    public:
                        ...
                        c_effect_instance_container()
                            : c_configuration_container("EffectInstance"),
                              m_name("Name", ""),
                              m_effect("Effect", ""),
                              m_render_stage("RenderStage", 0),
                              m_activation_operation("ActivationOperation", 0),
                              m_quad(),
                              m_activation_controls("ActivationControl", []()
                                                    {
                                                        return c_activation_control_container();
                                                    }) { }
                    ...
                    class c_external_pp_settings_container : public Configuration::c_configuration_container
                    {
                    public:
                        ...
                        c_external_pp_settings_container()
                            : c_configuration_container("PostProcessing"),
                              m_shader_directory("ShaderDirectory", ""),
                              m_texture_directory("TextureDirectory", "") ,
                              m_shaders("Shader", []()
                                        {
                                            return c_shader_container();
                                        }) ,
                              m_effects("Effect", []()
                                        {
                                            return c_effect_container();
                                        }) ,
                              m_effect_instances("EffectInstance", []()
                                                 {
                                                     return c_effect_instance_container();
                                                 }) { }
...
#pragma endregion
                    ...
                    bool c_system_external::ReadShaders(const std::vector<c_shader_container>& shaders)
                    {
                        ...
                        {
                            auto& set = m_members_external.m_shaders.shader_sets[index];
                    ...
                    void c_system_external::DeleteShaders()
                    {
                        for (uint32 i = 0; i < m_members_external.m_shaders.count; i++)
                        {
                            auto& set = m_members_external.m_shaders.shader_sets[i];
                    ...
                    bool c_system_external::ReadEffects(const std::vector<c_effect_container>& effects)
                    {
                        ...
                        {
                            auto& set = m_members_external.m_effects.effect_sets[index];
                            ...
                            for (i = 0; i < set.definition->shader_ids.Count; i++)
                            {
                                auto& shader_instance = set.shader_instances[i];
                                auto& id = set.definition->shader_ids[i];
                    ...
                    void c_system_external::DeleteEffects()
                    {
                        ...
                        {
                            auto& set = m_members_external.m_effects.effect_sets[i];
                    ...
                    bool c_system_external::ReadEffectInstances(const std::vector<c_effect_instance_container>& effect_instances)
                    {
                        ...
                        {
                            auto& set = m_members_external.m_effect_instances.effect_instance_sets[index];
                    ...
                    void c_system_external::DeleteEffectInstances()
                    {
                        for (uint32 i = 0; i < m_members_external.m_effect_instances.count; i++)
                        {
                            auto& set = m_members_external.m_effect_instances.effect_instance_sets[i];
                    ...
                    void c_system_external::BuildShaderDefinition(TagGroups::s_shader_postprocess_generic* definition, const char* shader_path)
                    {
                        ...
                        do
                        {
                            ...
                            uint32 data_size = 0;
                            auto data_pointer = shader_data.GetData(data_size);
                            ...
                            auto success = D3DXCreateEffectCompiler(CAST_PTR(const char*, data_pointer),
                                                                                                       data_size,
                            ...
                            {
                                auto handle = compiler->GetTechnique(i);
                                if (handle == nullptr)
                                {
                                    continue;
                                }
                    ...
                    void c_system_external::BuildTechnique(LPD3DXEFFECTCOMPILER compiler, D3DXHANDLE handle, TagGroups::s_technique_definition& technique)
                    {
                        ...
                        auto success = compiler->GetTechniqueDesc(handle, &technique_desc);
                        ...
                        {
                            auto pass = compiler->GetPass(handle, i);
                            if (pass == nullptr)
                            {
                                continue;
                            }
                    ...
                    void c_system_external::BuildPass(LPD3DXEFFECTCOMPILER compiler, D3DXHANDLE handle, TagGroups::s_pass_definition& pass)
                    {
                        ...
                        auto success = compiler->GetPassDesc(handle, &pass_desc);
                        ...
                        pass.render_chain = static_cast<_enum>(render_chain);
                    ...
                    void c_system_external::BuildParameters(LPD3DXEFFECTCOMPILER compiler, TagGroups::s_shader_postprocess_generic* definition)
                    {
                        ...
                        auto current = parameter_list;
                        for (auto i = 0; (i < definition->parameters.Count) && current; i++)
                        {
                    ...
                    void c_system_external::BuildEffectDefinition(s_effect_postprocess_external* definition, const c_effect_container& effect)
                    {
                        ...
                        {
                            auto& shader_id = definition->shader_ids[index];
                        ...
                        , const c_effect_instance_container& effect_instance)
                    {
                        ...
                        if (activation_control_count != 0)
                        {
                            ...
                            {
                                auto& control = definition->activation_controls[index];
                        ...
                        const int16 parameter_type_count)
                    {
                        ...
                        for (uint32 i = 0; i < count; i++)
                        {
                            ...
                            auto handle = compiler->GetParameterBySemantic(nullptr, semantic);
                            if (!handle)
                            {
                                continue;
                            }

                            auto parameter = new s_parameter_handle();
                        ...
                        TagGroups::s_shader_postprocess_parameter& parameter)
                    {
                        ...
                        auto success = compiler->GetParameterDesc(handle.handle, &parameter_desc);
                        ...
                        TagGroups::s_shader_postprocess_parameter& parameter)
                    {
                        ...
                        switch (handle.type.type)
                        {
                            ...
                            case Enums::_shader_variable_base_type_integer:
                                ...
                                if (SUCCEEDED(success))
                                {
                                    parameter.value.integer32.upper_bound = integer;
                                }

                                success = compiler->GetInt(value_lower_bound, &integer);
                                if (SUCCEEDED(success))
                                {
                                    parameter.value.integer32.lower_bound = integer;
                                }
                                break;
                        ...
                        TagGroups::s_shader_postprocess_parameter& parameter)
                    {
                        ...
                        auto value = 0;
                        auto success = compiler->GetInt(runtime_value_override, &value);
                        if (FAILED(success))
                        {
                            return;
                        }
                        ...
                        switch (handle.type.type)
                        {
                            case Enums::_shader_variable_base_type_boolean: valid = (value < Enums::_parameter_runtime_value_bool);
                                break;
                            case Enums::_shader_variable_base_type_integer: valid = (value < Enums::_parameter_runtime_value_int);
                                break;
                            case Enums::_shader_variable_base_type_argb_color: valid = (value < Enums::_parameter_runtime_value_color);
                                break;
                            case Enums::_shader_variable_base_type_float:
                            {
                                switch (handle.type.count)
                                {
                                    case 1: valid = (value < Enums::_parameter_runtime_value_float);
                                        break;
                                    case 2: valid = (value < Enums::_parameter_runtime_value_float2);
                                        break;
                                    case 3: valid = (value < Enums::_parameter_runtime_value_float3);
                                        break;
                                    case 4: valid = (value < Enums::_parameter_runtime_value_float4);
                                        break;
                                };
                            }
                                break;
                        ...
                        TagGroups::s_shader_postprocess_parameter& parameter)
                    {
                        ...
                        if (!parameter.animation_function.function)
                        {
                            return;
                        }
                        ...
                        if (SUCCEEDED(success))
                        {
                            parameter.animation_function.animation_duration = real32;
                        }
                        ...
                        if (SUCCEEDED(success))
                        {
                            parameter.animation_function.animation_rate = real32;
                        }
                    }
                        ...
                        TagGroups::s_shader_postprocess_parameter& parameter)
                    {
                        ...
                        const char* location;
                        auto success = compiler->GetString(texture_location, &location);
                    ...
                    {
                        // should the buffer be clear on this pass
                        auto annotation_handle = compiler->GetAnnotationByName(variable, annotation);
                    ...
                    void c_system_external::SetupRenderSets()
                    {
                        ...
                        for (auto i = 0; i < NUMBEROF(m_members_external.m_render_sets); i++)
                        {
                            m_members_external.m_render_sets[i].Ctor();
                            SetRenderSet(m_members_external.m_render_sets[i], static_cast<Enums::postprocess_render_stage>(i));
                        }
                    }
