            ...
            namespace Generic
            {
                namespace Internal
                {
                    ...
                    int16 c_effect_internal::GetScriptedVariableIndex(const char* name)
                    {
                        ...
                        for (auto i = 0; i < m_members_internal.definition->script_variables.Count; i++)
                        {
                    ...
                    void c_effect_internal::Ctor()
                    {
                        c_effect_generic::Ctor();

                        m_members_internal.definition = nullptr;
                    }

                    void c_effect_internal::Dtor()
                    {
                    ...
                    void c_effect_internal::SetupScriptedVariables()
                    {
                        for (auto i = 0; i < m_members_internal.definition->script_variables.Count; i++)
                        {
                            auto& variable = m_members_internal.definition->script_variables[i];
                    ...
                    void c_effect_internal::SetScriptedVariable(int16 index, real change_time, bool value)
                    {
                        ...
                        auto parameter = CAST_PTR(c_parameter_instance*, m_members_internal.definition->script_variables[index].runtime.exposed_parameter);
                    ...
                    void c_effect_internal::SetScriptedVariable(int16 index, real change_time, real value0, real value1, real value2, real value3)
                    {
                        ...
                        auto parameter = CAST_PTR(c_parameter_instance*, m_members_internal.definition->script_variables[index].runtime.exposed_parameter);
                        auto parameter_block = parameter->GetParameter();
                        ...
                        switch (parameter_block->value_type.count)
                        {
                            case 1:
                                ...
                                break;
                            case 2: parameter->SetOverrideInterp(vector2d, change_time);
                                break;
                            case 3: parameter->SetOverrideInterp(vector3d, change_time);
                                break;
                            case 4: parameter->SetOverrideInterp(quaternion, change_time);
                                break;
                        }
                    }
                };
