        ...
        namespace PostProcessing
        {
            namespace ErrorReporting
            {
                void Write(cstring string)
                {
                    ...
                    auto previous_dev_mode = *GameState::DeveloperMode();
                ...
                void WriteLine(cstring format, ...)
                {
                    ...
                    auto length = static_cast<size_t>(_vscprintf(format, argptr)) + 1;
                ...
                void WriteD3DXErrors(LPD3DXBUFFER buffer)
                {
                    ...
                    auto string_pointer = CAST_PTR(char*, buffer->GetBufferPointer());

                    auto remove_path = false;
                    auto work_dir_path_len = strlen(Settings::WorkingDirectoryPath());

                    auto string_length = strlen(string_pointer);
                    int32 count = 0;
                    while (string_pointer[0] != '\0' && count < k_max_error_line_count)
                    {
                        ...
                        else
                        {
                            ...
                            auto line_start = new char[work_dir_path_len + 1];
                            line_start[0] = 0;
                        ...
                        auto line_length = strcspn(string_pointer, "\n");

                        // copy the line to a new buffer
                        auto line_buffer = new char[line_length + 1];
