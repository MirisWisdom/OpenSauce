...
#define TAG_GROUP_STRING_TABLE_DEFINE(name, count, ...)			\
	...
	static_assert(NUMBEROF(name##_strings)==count,				\
		"too few strings defined for " #name);					\
	static Yelo::string_list name = {							\
		NUMBEROF(name##_strings),								\
		name##_strings											\
	}
...
#define TAG_GROUP_REFERENCES_DEFINE(name, flags, ...)						\
	static Yelo::cstring TAG_GROUP_REFERENCE_GET(name)##_tags[] = {			\
		__VA_ARGS__,														\
		NONE,																\
	};																		\
	static_assert(NUMBEROF(TAG_GROUP_REFERENCE_GET(name)) > 1,				\
