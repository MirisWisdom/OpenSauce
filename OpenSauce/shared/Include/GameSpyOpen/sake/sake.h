...
//		performed and indicated that the GameSpy services are available
//		for this game title and the Core object must have been initialized 
//		by calling gsCoreInitialize. Sample code for this is available
//		in the Sake test app.
//		<p />
//		The SAKE object initialized by this startup call is valid until the game 
//		shutdowns the Sake SDK with sakeShutdown.
// See Also
//		SAKEStartupResult
SAKEStartupResult SAKE_CALL sakeStartup
(
	SAKE *sakePtr
);
...
//								AuthService.
// Remarks
//		The profile id, cert, and proof are obtained from the
//		AuthService callback (after calling wsLoginUnique eg).
//		<p />
...
typedef enum
{
	...
	SAKEFieldType_INT64,
	SAKEFieldType_NULL,  // field is set to null - should only happen for Atlas stats with no default set
...
// Summary
//		This enum is used to determine the type local storage 
//      that the SDK will use to upload/update from or download to
//      from the backend.
// Remarks
...
typedef enum
{
	...
	SAKERequestResult_NOT_RATEABLE,				// The record is in a table that is not rateable (The ëRateable' flag
...
// SAKEContentInfo
// Summary
//		Structure to pass all the related information regarding content
//      to the API calls. 
// Remarks
...
// Summary
//		This typedef defines the progress callback function for the upload content API.
//      It is invoked periodically to indicate data transfer progress.
// Parameters
//      sake            : [in] The sake instance the callback is running on.
...
typedef void (*SAKEUploadContentProgressCallback)(SAKE      sake, 
...
// SAKEUploadContentCompletedCallback
// Summary
//		This typedef defines the callback function called when the Upload Content request
//      completes. After this is called if successful, 
//		the data is uploaded to the back end and a new file id is assigned.
// Parameters
//      sake        : [in] The sake instance the callback is running on.
...
typedef void (*SAKEUploadContentCompletedCallback)(SAKE     sake, 
	                                               SAKERequestResult result,
	                                               gsi_i32  fileId, 
...
// Summary
//		This typedef defines the progress callback function for the download content API.
//      It is invoked periodically to indicate data transfer progress.
// Parameters
//      sake            : [in] The sake instance the callback is running on.
...
typedef void (*SAKEDownloadContentProgressCallback)(SAKE    sake, 
                                                    gsi_u32 bytesTransfered, 
                                                    gsi_u32 totalSize, 
                                                    void    *userData);
...
// SAKEDownloadContentCompletedCallback
// Summary
//		This typedef defines the callback function called when the Download Content request
//      completes. After this is called if successful, the content is downloaded to the specified
//      memory location or to a file
// Parameters
//      sake        : [in] The sake instance the callback is running on.
//		result	    : [in] The download file result as it is returned by the service call.
//		buffer	    : [in] The buffer which contains the downloaded content, if downloading to memory.
//                         Otherwise, it is NULL.
//		bufferLength: [in] The buffer length for the downloaded content, if downloading to memory.
//                         Otherwise, it is 0.
//		userData    : [in/out] The developer specified void pointer data shared 
//                         between the API and its callback. 
// Remarks
//      IMPORTANT: If downloading to memory developer must delete the allocated memory by the
//      callback when the buffer is no longer needed.
// See Also
//		SAKEFileResult, SAKEDownloadContentProgressCallback
typedef void (*SAKEDownloadContentCompletedCallback)(SAKE    sake, 
                                                     SAKERequestResult result,
                                                     SAKEFileResult fileResult, 
                                                     char    *buffer, 
...
//		was attempted, so it must be called immediately after a failure 
//		to get the reason for that failure.
// See Also
//		SAKEStartRequestResult
SAKEStartRequestResult SAKE_CALL sakeGetStartRequestResult
(
	SAKE sake
);
...
// Remarks
//      This may only be called for a table with the Content Moderation feature enabled. 
//      Contact your account manager if you are interested in Content Moderation.
// See Also
//		SAKEReportRecordInput, SAKERequestCallback
SAKERequest SAKE_CALL sakeReportRecord
(
 SAKE					sake, 
...
//		contains the recordid of the newly created record.
// See Also
...
//		service does not send any response other than the success or 
//		failure indicated by the result parameter passed to the callback. 
//		When the callback is called, the outputData parameter will always 
//		be set to NULL.
// See Also
//		sakeUpdateRecord
typedef struct
...
typedef struct
{
	...
	int       *mRecordIds;		// An array of recordids, each one identifying a record which is 
								// to be returned.
...
// Remarks
//		If the request completed successfully, then the output object
//		contains all of the records which were specified in the request. 
//		See the definitions of the Input & Output structs for more 
//		information about how to limit what is retrieved in the request
//		and certain metadeta fields that can be retrieved.
// See Also
...
//		pass the filter, in which case the output will contain NULL data 
//		for the returned record). Note that this function works best 
//		in a table in which records are not deleted or are deleted in
//		order of oldest first (in other words, in tables in which 
//		recordids are contiguous).
// See Also
...
//		However, a game can restrict itself to a subset of that 
//		range if it wishes. 
//		<p />
//		For example, a game may want to use a rating scale of 1 to 
//		5 (e.g., stars), or it may want to use a range of 0 to 100. 
//		Sake allows users to rate records which they own, however 
//		no player profile can rate a single record more than once. 
//		<p />
//		Internally, Sake stores every rating given by every user, 
//		which allows it to compute accurate averages and to prevent 
//		repeat ratings. The field name "my_rating" can be used to 
//		obtain the current profile's rating for a given record or when 
//		searching for records. By default, my_rating = -1 for records 
//		the current user has not yet rated. When browsing for records, 
//		you can use the special search tags @rated or @unrated in the 
//		filter string to limit the searches to either rated or unrated 
//		records.
// See Also
//		sakeRateRecord
typedef struct
...
//		profile can rate the same record more than once.
// See Also
...
//		first call for a specific leaderboard the information this function returns
//		will be the same so it should be cached for subsequent page calls.
// See Also
...
// See Also
//		SAKEField
SAKEField * SAKE_CALL sakeGetFieldByName
(
	const char	*name, 
	SAKEField	*fields, 
	int			numFields
...
// sakeCancelRequest
// Summary
//		Used for cancelling any outstanding requests made to the Sake Storage Server.
// Parameters
//		sake		: [in] The sake object.
//		request		: [in] The returned value from the original request to be cancelled.  
// Returns
//		SAKEStartRequestResult_SUCCESS if successful.
// Remarks
//		To be used for cancelling requests which taking too long or unresponsive. No actions are cancelled 
//      at the backend if the request was processed and response was not received yet. The cancel request only
//      to prevent the SDK waiting indefinitely for a response from the backend.
...
gsi_bool SAKE_CALL sakeSetFileDownloadURL
(
	SAKE		sake, 
...
gsi_bool SAKE_CALL sakeSetFileUploadURL
(
	SAKE		sake, 
	gsi_char	url[SAKE_MAX_URL_LENGTH]
);
...
// Parameters
//		sake		: [in] The Sake object. 
//		fileid		: [in] The fileid returned by the headers of the uploaded file. 
...
SAKEStartRequestResult SAKE_CALL sakeUploadContent
(
    SAKE                    sake, 
...
typedef struct SAKEDownloadContentInput
{
    ...
    gsi_bool transferBlocking;      // gsi_true if request is a blocking call
    SAKEDownloadContentProgressCallback  progressCallback; // developer's callback to tack download progress
} SAKEDownloadContentInput;
...
SAKEStartRequestResult SAKE_CALL sakeDownloadContent
(
    SAKE                     sake, 
...
// Parameters
//		headers		: [in] The headers to parse for the fileid. You can get these 
//							from ghttpGetHeaders. 
...
//		otherwise.
// Remarks
//      You can also check the headers manually for the ìSake-File-ResultÅEheader. 
//		The value stored in the header is an integer, the possible values of 
//		which are enumerated in SAKEFileResult. SAKEFileResult_SUCCESS means that 
//		the file was uploaded successfully, while any other value indicates that 
...
// sakeGetFileIdFromHeaders
// Summary
//		If the file was uploaded successfully, this function obtains 
//		the fileid that references the file.
// Parameters
//		headers		: [in] The headers to parse for the fileid. You can get these 
//							by calling ghttpGetHeaders. 
//		fileId		: [ref] The fileid returned by SakeFileServer when the file
//							was uploaded. 
// Returns
//		gsi_true if able to parse the fileid successfully, gsi_false otherwise.
// Remarks
//      To get the fileid from the headers manually, look for the ìSake-File-IdÅE
//		header. Once obtained, the fileid can now be stored in a fileid field 
//		in the database. 
//		<p />
//		Note: If you upload a file, but do not store the fileid in the database, 
...
// sakeThink
// Summary
//		This function is used for processing the ghttp request
//      and responses. It should be called in the main loop.
void sakeThink();
