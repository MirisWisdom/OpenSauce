	...
	*outputRecordsPtr = outputRecords;
	...
	for(recordIndex = 0 ; recordIndex < *numRecords ; recordIndex++)
	{
		...
		for(fieldIndex = 0 ; fieldIndex < numFields ; fieldIndex++)
		{
			...
			{
				// only in the case of a null field should we get an empty RecordValue in the SOAP response
				// (and this case should only happen if an Atlas 2.0 stat has no default set)
