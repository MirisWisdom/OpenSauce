	...
	namespace GameState
	{
		namespace RuntimeData
		{
			struct s_runtime_data
			{
				...
				struct s_integers : public s_value_array<int32> { } integers;

				struct s_vectors : public s_value_array<real_vector3d> { } vectors;
			};
			...
			// Validate that an operation can be performed at [value_index]
			static bool IntegerOperationValidate(
				int16 value_index)
			{
			...
			// Returns NONE if this operation fails.
			static int32 IntegerGetAndIncrement(
				int16 value_index)
			{
				if (IntegerOperationValidate(value_index))
					return runtime_data->integers.values[value_index]++;
			...
			// Returns NONE if this operation fails.
			static int32 IntegerGetAndDecrement(
				int16 value_index)
			{
				if (IntegerOperationValidate(value_index))
					return runtime_data->integers.values[value_index]--;
			...
			static int32 IntegerDoOperation(
				int16 value_index,
				cstring op_name,
				int32 op_value)
			{
				...
				if (IntegerOperationValidate(value_index))
				{
					...
					//////////////////////////////////////////////////////////////////////////
					// mutable
					if (__INTEGER_OP_CASE("="))
						result = value = op_value;
					else if (__INTEGER_OP_CASE("+="))
						result = value += op_value;
					else if (__INTEGER_OP_CASE("-="))
						result = value -= op_value;
					else if (__INTEGER_OP_CASE("*="))
						result = value *= op_value;
					else if (__INTEGER_OP_CASE_ARG_NONZERO("/="))
						result = value /= op_value;
					else if (__INTEGER_OP_CASE("++"))
						result = value++;
					else if (__INTEGER_OP_CASE("--"))
						...
						result = value ^= op_value;
					else if (__INTEGER_OP_CASE("<<=") && valid_bs)
						result = value <<= op_value;
					else if (__INTEGER_OP_CASE(">>=") && valid_bs)
						result = value >>= op_value;
						//////////////////////////////////////////////////////////////////////////
			...
			// Validate that an operation can be performed at [value_index]
			static bool VectorOperationValidate(
				int16 value_index,
				int16 element_index = 0)
			{
				...
				real op_value)
			{
				...
				if (VectorOperationValidate(value_index, element_index))
				{
					...
					if (__VECTOR_OP_CASE("="))
						...
						*element_ptr *= op_value;
					else if (__VECTOR_OP_CASE("/=") && op_value != 0.0f)
						*element_ptr /= op_value;
					else
						result = false;
				}
				...
				int16 op_arg_vector_index)
			{
				...
				if (VectorOperationValidate(value_index))
				{
					...
					real_vector3d* vec_rhs = VectorOperationValidate(op_arg_vector_index)
						? &runtime_data->vectors.values[op_arg_vector_index]
						: nullptr;

					result = true;

					if (__VECTOR_OP_CASE("normalize"))
						vector.Normalize();
					else if (__VECTOR_OP_CASE("inverse"))
						vector.Inverse();
					else if (__VECTOR_OP_CASE("conjugate"))
						vector = -vector;
					else if (__VECTOR_OP_CASE_ARG("+="))
						vector += *vec_rhs;
					else if (__VECTOR_OP_CASE_ARG("-="))
						vector -= *vec_rhs;
					else if (__VECTOR_OP_CASE_ARG("*="))
						vector *= vec_rhs->i;
					else if (__VECTOR_OP_CASE_ARG_I_NONZERO("/="))
						vector /= vec_rhs->i;
					else if (__VECTOR_OP_CASE_ARG("cross_product"))
						vector = vector ^ *vec_rhs;
					else if (__VECTOR_OP_CASE_ARG("dot_product"))
						vec_rhs->i = vector * *vec_rhs;
					else if (__VECTOR_OP_CASE_ARG("scalar_mul"))
						vector = vector * vec_rhs->i;
					else if (__VECTOR_OP_CASE_ARG_I_NONZERO("scalar_div"))
						vector = vector / vec_rhs->i;
					else if (__VECTOR_OP_CASE_ARG("magnitude"))
						vec_rhs->i = vector.Magnitude();
					else
						result = false;
				}
			...
			static void* scripting_runtime_integer_get_evaluate(
				void** arguments)
			{
				struct s_arguments
				{
			...
			static void* scripting_runtime_integer_set_evaluate(
				void** arguments)
			{
				struct s_arguments
				{
			...
			static void* scripting_runtime_integer_inc_evaluate(
				void** arguments)
			{
				struct s_arguments
				{
			...
			static void* scripting_runtime_integer_dec_evaluate(
				void** arguments)
			{
				struct s_arguments
				{
			...
			static void* scripting_runtime_integer_operation_evaluate(
				void** arguments)
			{
				struct s_arguments
				{
			...
			static void* scripting_runtime_vector_get_element_evaluate(
				void** arguments)
			{
				struct s_arguments
				{
			...
			static void* scripting_runtime_vector_set_element_evaluate(
				void** arguments)
			{
				struct s_arguments
				{
				...
				result.boolean = VectorSetElement(
					args->value_index,
					args->element_index,
					args->op_name,
					args->op_value);
			...
			static void* scripting_runtime_vector_set_evaluate(
				void** arguments)
			{
				struct s_arguments
				{
			...
			static void* scripting_runtime_vector_operation_evaluate(
				void** arguments)
			{
				struct s_arguments
				{
			...
			static void* scripting_runtime_vector_to_string_evaluate(
				void** arguments)
			{
				struct s_arguments
				{
			...
			const real_vector3d* VectorValueGet(
				int16 value_index)
			{
				if (VectorOperationValidate(value_index))
					return &runtime_data->vectors.values[value_index];
