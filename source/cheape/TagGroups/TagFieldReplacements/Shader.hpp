...
#pragma region Shader Effect
		...
		static void InitialiseShaderEffect(tag_block_definition* block_definition)
		{
			TAG_GROUP_BLOCK_FIELDS_DEFINE(shader_effect_extension) =
			{
				...
				// Depth fade
				TAG_FIELD_ENTRY(explanation, "Fade Controls", "Controls the softness of an effect"),

				TAG_FIELD_ENTRY(real, "depth fade distance", "Controls the softness of an effect when intersecting opaque geometry"),
				TAG_FIELD_ENTRY(real, "camera fade distance", "Controls the softness of an effect when nearing the near clip plane"),
		...
		static void InitialiseShaderEffect_WeatherParticleSystem()
		{
			...
			auto block_definition = Shared::GetTagField(*rain->header_block_definition, e_field_type::block, "particle types").get_definition<tag_block_definition>();
		...
		static void InitialiseShaderEffect_ParticleSystem()
		{
			...
			auto block_definition = Shared::GetTagField(*pctl->header_block_definition, e_field_type::block, "particle types:particle states").get_definition<tag_block_definition>();
...
#pragma region Shader Environment
		static void InitializeShaderEnvironment()
		{
			...
			{
				TAG_FIELD_ENTRY(explanation, "directional lightmaps", "" ),
				TAG_FIELD_ENTRY(word_flags, "flags", &shader_environment_extension_flags),
				TAG_FIELD_ENTRY_PAD(sizeof(int16)),
				TAG_FIELD_ENTRY(real, "bump amount", nullptr),

				TAG_FIELD_ENTRY(explanation, "specular DLM color map", "" ),
				TAG_FIELD_ENTRY(tag_reference, "specular color map", &Shared::TAG_GROUP_REFERENCE_GET(bitmap)),
				TAG_FIELD_ENTRY(real, "specular color exponent", nullptr),
				TAG_FIELD_ENTRY(real, "specular color coefficient", nullptr),
				TAG_FIELD_ENTRY(word_flags, "specular color flags", &shader_environment_extension_specular_flags),
				TAG_FIELD_ENTRY_PAD(sizeof(int16)),
				
				TAG_FIELD_ENTRY(explanation, "specular DLM tint and brightness", "" ),
				TAG_FIELD_ENTRY(real_fraction, "perpendicular brightness", nullptr),
				TAG_FIELD_ENTRY(real_rgb_color, "perpendicular tint color", nullptr),
				TAG_FIELD_ENTRY(real_fraction, "parallel brightness", nullptr),
				TAG_FIELD_ENTRY(real_rgb_color, "parallel tint color", nullptr),
				
				TAG_FIELD_ENTRY(explanation, "specular DLM intensity", "" ),
				TAG_FIELD_ENTRY(real, "specular lighting exponent", nullptr),
				TAG_FIELD_ENTRY(real, "specular lighting coefficient", nullptr),
