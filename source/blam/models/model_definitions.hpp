...
#include <yelo/tag_files/TagBlock.h>
...
namespace Yelo
{
	...
	namespace TagGroups
	{
		...
		}; ASSERT_SIZE(model_marker_instance, 0x20); // max count: 32
		...
		}; ASSERT_SIZE(model_markers, 0x40); // max count: 256
		...
		}; ASSERT_SIZE(model_node, 0x9C); // max count: 64
		...
		struct model_region_permutation_marker
		{
			...
			TAG_FIELD(real_point3d, translation);
			TAG_PAD(int32, 4);
		}; ASSERT_SIZE(model_region_permutation_marker, 0x50); // max count: 64
		...
		}; ASSERT_SIZE(gbxmodel_region_permutation, 0x58); // max count: 32
		struct gbxmodel_region
		{
			TAG_FIELD(tag_string, name);
			TAG_PAD(int32, 8);
			TAG_TBLOCK(permutations, gbxmodel_region_permutation);
		}; ASSERT_SIZE(gbxmodel_region, 0x4C); // max count: 32
		...
		}; ASSERT_SIZE(model_triangle, 0x6); // max count: 65535
		struct gbxmodel_geometry_part
		{
			...
			PAD16;
		}; ASSERT_SIZE(gbxmodel_geometry_part, 0x84); // max count: 32
		...
		}; ASSERT_SIZE(gbxmodel_geometry, 0x30); // max count: 256
		...
		struct model_shader_reference
		{
			...
			PAD16;
			TAG_PAD(int32, 3);
		}; ASSERT_SIZE(model_shader_reference, 0x20); // max count: 64
		...
		}; ASSERT_SIZE(gbxmodel_definition, 0xE8); // max count: 1
