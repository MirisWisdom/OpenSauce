...
VS_OUTPUT main(float4 v4 : TEXCOORD0, float4 v0 : POSITION0, float3 v1 : NORMAL0, float3 v2 : BINORMAL0, float3 v3 : TANGENT0)
{
	...
	// (12) output texcoords -----------------------------------------------------------------
	R_TEMP0.x = dot(V_POSITION, c_world_view_projection[0]);	
	R_TEMP0.y = dot(V_POSITION, c_world_view_projection[1]);	
	R_TEMP0.zw = dot(V_POSITION, c_world_view_projection[3]);	
...
Technique transparent_generic_screenspace
{
	Pass P0
	{
