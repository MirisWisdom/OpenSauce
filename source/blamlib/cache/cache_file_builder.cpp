...
namespace Yelo
{
	namespace Cache
	{
		static void fix_game_globals(
			const datum_index globals_index,
			const Enums::scenario_type scenario_type)
		{
		...
		static void fix_scenario_yelo(
			const datum_index project_yellow_index)
		{
			...
			blam::tag_get<TagGroups::project_yellow>(project_yellow_index)->LoadGameGlobalsOverride();
		}

		static void initialize_yelo_cache_header(
			const datum_index project_yellow_index)
		{
			if (project_yellow_index.IsNull())
			{
			...
			const cstring scenario_name)
		{
			...
			auto scenario_index = blam::tag_load<TagGroups::scenario>(
				scenario_name,
			...
			if (build_cache_file_for_yelo())
			{
				const auto& yelo_reference = scenario->GetYeloReferenceHack();

				fix_scenario_yelo(yelo_reference.tag_index);
				initialize_yelo_cache_header(yelo_reference.tag_index);
			}
			...
			auto globals_index = blam::tag_load<TagGroups::s_game_globals>(
				Scenario::K_GAME_GLOBALS_TAG_NAME,
				FLAGS_T(e_tag_load_flags, e_tag_load_flags::from_file_system_bit, e_tag_load_flags::non_resolving_references_bit));
			// the engine code returns true even if the tags fail to load
			...
			fix_game_globals(globals_index, scenario->type);
	...
	namespace blam
	{
		...
		template <typename T>
		static T* stream_blob_to_buffer(
			T*& stream,
			const void* blob,
		...
		void* stream_tag_block_to_buffer(
			tag_block* block,
			void* stream,
		...
		static void* stream_tag_block_to_buffer_postprocess_tag_data(
			void*& return_stream,
			const uintptr_t stream_base_address,
			const uintptr_t virtual_base_address,
			build_cache_file_tag_names_t& tag_names,
			const TagGroups::c_tag_field_scanner& scanner)
		{
			...
			else if (!data->definition->flags.test(e_tag_data_flags::not_streamed_to_cache_bit))
			{
				if (data->size > 0 && data->address != nullptr)
				{
					...
					void* data_address = RebasePointer(return_stream, stream_base_address, virtual_base_address);
			...
			const TagGroups::c_tag_field_scanner& scanner)
		{
			...
			if (block->count > 0)
			{
				...
				void* elements_address = RebasePointer(return_stream, stream_base_address, virtual_base_address);
				return_stream = stream_tag_block_to_buffer(
					block,
					return_stream,
					stream_base_address,
			...
			const TagGroups::c_tag_field_scanner& scanner)
		{
			...
			YELO_ASSERT( reference->tag_index==datum_index::null ||
				(absolute_index>=0 &&
					(absolute_index<Enums::k_maximum_simultaneous_tag_instances || absolute_index<Enums::
		...
		static void* stream_tag_block_to_buffer(
			tag_block* block,
			void* stream,
			uintptr_t stream_base_address,
			uintptr_t virtual_base_address,
			build_cache_file_tag_names_t& tag_names)
		{
			...
			void* return_stream = stream_blob_to_buffer(
				elements_stream,
				block->address,
				block->count * block->get_element_size());

			for (auto element : *block)
			{
				TagGroups::c_tag_field_scanner scanner(
		...
		void* stream_tag_to_buffer(
			datum_index tag_index,
			void* stream,
			size_t& return_tag_size,
			uintptr_t virtual_base_address,
			build_cache_file_tag_names_t& tag_names)
		{
			tag_block* block = tag_get_root_block(tag_index);

			void* return_stream = stream_tag_block_to_buffer(
				block,
				stream,
				CAST_PTR(uintptr_t, stream),
				virtual_base_address,
				tag_names);
		...
		static size_t stream_structure_bsp_to_buffer(
			TagGroups::scenario_structure_bsp_reference& reference,
			void* stream,
			build_cache_file_tag_names_t& tag_names,
			uintptr_t virtual_base_address)
		{
			...
			size_t bsp_tag_size;
			stream_tag_to_buffer(
				reference.structure_bsp.tag_index,
				bsp_stream,
				bsp_tag_size,
				virtual_base_address,
				tag_names);
			...
			bsp_stream_size = AlignValue(bsp_stream_size, Cache::k_cache_file_page_alignment_bit);
		...
		static API_FUNC_NAKED void* PLATFORM_API build_cache_file_add_model_vertices_and_triangles(
			s_cache_tag_header* cache_tag_header,
			void* scratch)
		{
		...
		static void build_cache_file_add_model_vertices_and_triangles_resources(
			s_cache_tag_header* cache_tag_header)
		{
			...
			if (!build_cache_file_add_resource(
				vertex_y_index_buffer,
				cache_tag_header->model_data_size,
		...
		static char* build_cache_file_add_tag_to_header(
			datum_index tag_index,
			uintptr_t stream_base_address,
			s_cache_tag_instance* tag_instances,
			int32 tag_instance_index,
			char* tag_name,
			build_cache_file_tag_names_t& tag_names)
		{
			...
			{
				const int32 MAXIMUM_SIMULTANEOUS_TAG_INSTANCES = build_cache_file_for_yelo()
		...
		static void* build_cache_file_add_tag_to_stream(
			datum_index tag_index,
			uintptr_t stream_base_address,
			s_cache_tag_instance* tag_instances,
			int32 tag_instance_index,
			...
			build_cache_file_tag_names_t& tag_names)
		{
			...
			switch (tag_instance->group_tag)
			{
				...
				default_case:
					...
					void* tag_base_address = stream_tag_to_buffer(
						tag_index,
						stream,
						tag_stream_size,
						Enums::k_tag_base_address,
						tag_names);
		...
		static bool PLATFORM_API build_cache_file_add_tags_impl(
			...
			void* scratch,
			build_cache_file_tag_names_t& tag_names,
			int32 largest_structure_bsp_size)
		{
			enum
			{
			...
			for (datum_index tag_index : TagGroups::c_tag_iterator::all())
			{
				...
				next_cache_tag_name = build_cache_file_add_tag_to_header(
					tag_index,
					CAST_PTR(uintptr_t, cache_tag_header),
					cache_tag_instances,
					index,
					next_cache_tag_name,
					tag_names);
			...
			printf_s(
				"tag headers and names are %3.2fM\n",
			...
			for (datum_index tag_index : TagGroups::c_tag_iterator::all())
			{
				...
				next_cache_tag_base_address = build_cache_file_add_tag_to_stream(
					tag_index,
					CAST_PTR(uintptr_t, cache_tag_header),
					cache_tag_instances,
					index,
					next_cache_tag_base_address,
					tag_names);
			}
			printf_s("done\n");
		...
		static bool build_cache_file_add_structure_bsps_impl(
			void* stream,
			build_cache_file_tag_names_t& tag_names,
			_Out_			int32& out_largest_structure_bsp_size)
		{
			...
			for (auto& bsp_reference : scenario->structure_bsps)
			{
					...
					&bsp_reference.cache_offset))
				{
					success = false;
					printf_s(
						"failed to write the structure-bsp '%s' to the cache file",
						bsp_tag_name);
		...
		static bool build_cache_file_add_tags(
			build_cache_file_tag_names_t& tag_indexes,
			s_cache_header& cache_header,
			void* scratch)
		{
		...
		static bool build_cache_file_write_header_and_compress(
			s_cache_header& cache_header)
		{
			...
			if (!build_cache_file_for_yelo())
			{
				switch (cache_header.cache_type)
				{
					case Enums::_cache_file_type_campaign:
						maximum_cache_file_length = k_max_cache_size;
						break;
			...
			{
				printf_s(
					"cache file %.2fM too large (should be %.2fM but was %.2fM)\n",
		...
		void build_cache_file_for_scenario(
			const cstring scenario_path,
			const e_build_cache_file_begin::flags_t begin_flags)
		{
			...
			do
			{
				if (!scenario_load_for_cache_build(scenario_path) ||
			...
			if (!build_cache_file_globals()->canceled)
			{
				tag_groups_dump_memory();

				if (begin_flags.test(e_build_cache_file_begin::dump_tag_group_allocation_stats_bit))
				{
					using namespace TagGroups;
