...
extern "C" { // Mark as C code so the compiler doesn't mangle the exported function name with C++ markup
	...
	UTILDLL_API HRESULT Util_CalculateModuleCodeSize(IN LPCTSTR lpFileName, OUT PSIZE_T code_size);
	...
	UTILDLL_API HRESULT Util_RebaseModule(IN LPCTSTR lpFileName, OUT PBYTE reloc_dll, IN SIZE_T reloc_dll_size, 
		IN OUT PDWORD base_address, IN SIZE_T reloc_alloc_count);
};