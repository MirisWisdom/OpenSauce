...
float gDesat = 0.5f;
float gToned = 1.0f;
float3 gLightColor = { 1.0f, 0.9f, 0.5f };
float3 gDarkColor = { 0.2f, 0.05f, 0.0f };

float4 SepiaPS( float2 Tex0 : TEXCOORD0 ) : COLOR0{
	float3 scnColor = gLightColor * tex2D(sam_Source, Tex0).xyz;
	float3 grayXfer = float3(0.3,0.59,0.11);
	float gray = dot(grayXfer,scnColor);
	float3 muted = lerp(scnColor,gray.xxx,gDesat);
	float3 sepia = lerp(gDarkColor,gLightColor,gray);
