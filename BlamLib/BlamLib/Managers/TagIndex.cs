...
namespace BlamLib.Managers
{
	...
	public interface ITagIndex : IDisposable
	{
		...
		string StringIdGet(Blam.StringID handle);

		bool StringIdTryAndGet(string value, out Blam.StringID handle);
	...
	public abstract class TagIndexBase : ITagIndex, IEnumerable<TagManager>
	{
		...
		/// <param name="handle">Requested string id handle</param>
		/// <returns>Requested string id's value, or null if the tag index doesn't support string ids</returns>
		public abstract string StringIdGet(Blam.StringID handle);

		public abstract bool StringIdTryAndGet(string value, out Blam.StringID handle);
		#endregion
	...
	public sealed class TagIndex : TagIndexBase
	{
		...
		StringIdManager stringTable = null;
		/// <summary>
		/// Manager object for this index's tag definition's string id fields
		/// </summary>
		public StringIdManager StringTable	{ get { return stringTable; } }
		...
		{
			engine = version;
			directory = System.IO.Path.Combine(dir, "tags\\");
