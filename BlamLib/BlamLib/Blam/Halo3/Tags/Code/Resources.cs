			...
			internal byte[] GetSegmentData(CacheFileBase cf, cache_file_resource_layout_table owner, int segment_offset)
			{
				if (PageData == null)
				{
					...
					if (SharedCache.Value > -1) // above stmt disables this code for now
					{
						...
						bool is_internal;
						CacheFile scf = Program.Halo3.FromLocation(cf.EngineVersion, scb.CachePath.Value, out is_internal);
					...
					uint base_offset = (uint)cf.HeaderHalo3.Interop[Blam.CacheSectionType.Resource].CacheOffset;
